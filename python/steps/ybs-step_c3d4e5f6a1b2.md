...
Implements the `Packet` dataclass — the YX wire-format data structure with `hmac`, `guid`, and `payload` fields, plus `serialize()` and `deserialize()` methods in `src/yx/transport/packet.py`.
...
**Implements**: protocol/specs/technical/yx-protocol-spec.md § Wire Format (HMAC + GUID + Payload)
**References**: protocol/specs/testing/testing-strategy.md § Category 1: Unit Tests
...
Implements: protocol/specs/testing/testing-strategy.md § Category 1: Unit Tests
"""
...
```
STEP ybs-step_c3d4e5f6a1b2: Packet Data Structure
...
- src/yx/transport/packet.py
- tests/unit/test_packet.py

VERIFICATION: PASSED (attempt [N])

NEXT STEP: ybs-step_d4e5f6a1b2c3 (HMAC Computation)
```
...
- **Next**: `ybs-step_d4e5f6a1b2c3` — HMAC Computation
