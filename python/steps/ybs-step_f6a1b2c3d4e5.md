...
Extends `PacketBuilder` with `parse_packet()` and `validate_hmac()` — converting raw bytes back into verified `Packet` objects, rejecting tampered or malformed packets.
...
**Implements**: protocol/specs/technical/yx-protocol-spec.md § Packet Parsing/Validation
...
```
STEP ybs-step_f6a1b2c3d4e5: Packet Parser and Validator
...
- src/yx/transport/packet_builder.py (extended)
- tests/unit/test_packet_parser.py

VERIFICATION: PASSED (attempt [N])

NEXT STEP: ybs-step_a2b3c4d5e6f7 (UDP Socket Configuration)
```
...
- **Next**: `ybs-step_a2b3c4d5e6f7` — UDP Socket Configuration
