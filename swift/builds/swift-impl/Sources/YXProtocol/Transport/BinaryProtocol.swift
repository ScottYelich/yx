...
// Implements: protocol/specs/technical/yx-protocol-spec.md § Protocol 1 (Binary/Chunked)
// Header format (16 bytes): [proto(1)] + [protoOpts(1)] + [channelID(2)] + [sequence(4)] +
...
public enum BinaryProtocolError: Error {
    case missingKey
    case invalidFormat(String)
}

/// Protocol 1: Binary/Chunked handler
public actor BinaryProtocol {
    ...
    private var processedKeys: [UInt64: Date] = [:]
    ...
    public func encode(data: Data, protoOpts: UInt8 = 0x00, channelID: UInt16 = 0) throws -> [Data] {
        let sequence = nextSequence(channelID: channelID)
        var processed = data
        if protoOpts & 0x01 != 0 { processed = try processed.zlibCompress() }
        if protoOpts & 0x02 != 0 {
            guard let key = key else { throw BinaryProtocolError.missingKey }
        ...
        return try chunks.enumerated().map { (index, chunk) in
            try buildPayload(proto: Self.protocolID, protoOpts: protoOpts,
    ...
    private func parseHeader(_ payload: Data) -> BinaryHeader {
    ...
    private func buildPayload(proto: UInt8, protoOpts: UInt8,
    ...
    private func cleanupBuffers() {
        ...
        processedKeys = processedKeys.filter { $0.value > cutoff }
    }
}
