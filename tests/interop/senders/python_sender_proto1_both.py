#!/usr/bin/env python3
"""
Protocol 1 (compressed + encrypted) test sender.

Usage: python_sender_proto1_both.py <data_hex>
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python_sender_proto1_both.py <data_hex>")
        sys.exit(1)
    ...
    packets = SimplePacketBuilder.build_binary_packet(
        ...
        TestConfig.test_key(),
        proto_opts=0x03,  # Compressed + Encrypted
        channel_id=0,
        sequence=0
    )
