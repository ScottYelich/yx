...
Task {
    do {
        ...
        // Bind ONE persistent socket so back-to-back chunks are not dropped
        // between receives (the one-shot UDPHelper.receive would lose them).
        let sock = try UDPHelper.bindReceiver(port: TestConfig.testPort, timeout: 5.0)
        defer { UDPHelper.closeReceiver(sock) }

        // Receive chunks
        while receivedCount < maxChunks {
            let packet = try UDPHelper.receiveOn(sock)
            ...
            // Verify HMAC. Wrap slices in Data(...) to rebase indices to 0 —
            // Data slices keep absolute indices, so literal-offset slicing on a
            // slice traps (SIGTRAP) once startIndex != 0.
            ...
            // Extract payload (skip GUID); rebase to 0 so payload[0]/header loads work.
            ...
            // Extract data
            let data = Data(payload.suffix(from: 16))
            ...
            if await buffer.isComplete() {
                ...
                if opts & 0x02 != 0 {
                    // Use testKey (0x00) for AES to match the Python canonical
                    // reference, whose build_binary_packet encrypts with the
                    // shared HMAC/test key (not a separate encryption key).
                    let symmetricKey = try Data.symmetricKey(from: TestConfig.testKey)
